...
def _calculate_ssim(
    ...
    def scan_body(carry, xs):
        i = xs
        img1_c = lax.dynamic_slice_in_dim(img1, i, 1, axis=3)  # (batch, H, W, 1)
        img2_c = lax.dynamic_slice_in_dim(img2, i, 1, axis=3)  # (batch, H, W, 1)

        ssim_for_channel = _calculate_ssim_for_channel(
            img1_c, img2_c, kernel_conv, c1, c2
        )
        return carry, ssim_for_channel

    _, ssim_scores = jax.lax.scan(
        scan_body, 0, jnp.arange(num_channels, dtype=jnp.uint32)
    )
    # print("SHAPE ssim_scores", ssim_scores.shape)
    # ssim_scores: (num_channels, batch)
    # mean over channels: (batch,)
    return jnp.mean(ssim_scores, axis=0)
