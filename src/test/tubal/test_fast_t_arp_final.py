...
from t_arp.tubal import t_svd, t_rsvd, t_tsvd, reconstruct_t_svd
...
def sample_row(
    ...
    # index the row
    v = jax.lax.dynamic_index_in_dim(V.t_data, index=j_k, axis=0)
...
def test_t_arp(key, V, V_t_data, n_slices):
    ...
    for i in range(n_slices):
        ...
        # STANDARD T-ARP
        j_1, v_t_data = sample_row(subkey, V)
...
U, S, Vt = t_tsvd(M=M, n_slices=N_SLICES)
...
# Inspect T-ARP
n_slices = N_SLICES
J_1, J_2 = test_t_arp(key, V, V.t_data, n_slices)
print("STD list", J_1)
print("FST list", J_2)
